...
contract MaxApyZap {
    ...
    function maxIn(
        ...
        _approveMax(cachedVault, vaultAsset);
        uint256 _minSharesOut = maxInData.minSharesOut;
        assembly ("memory-safe") {
            ...
            mstore(0x20, _amount) // Append the `amount` argument
            mstore(0x40, _recipient) // Append the `recipient` argument
    ...
    /// @param maxInData the struct with the data necessary to call MaxApyVault
    ...
    function maxInNative(
        ...
        address vaultAsset = maxInData.vault.asset();
        if (cachedWrappedToken != maxInData.assetIn) revert AssetNotWETH();
        address cachedVault = address(maxInData.vault);
        ...
        uint256 _amount = _thisBalance(cachedWrappedToken);
        if (cachedWrappedToken != vaultAsset) {
            _swap(maxInData.router, maxInData.assetIn, maxInData.swapData);
            _amount = _thisBalance(vaultAsset);
        }
    ...
    function maxOut(
        ...
        if (_recipient == address(0)) revert InvalidRecipient();
        // Cache `wrappedToken` and `vault` due to assembly's immutable access restrictions
        address cachedVault = address(maxOutData.vault);
        uint256 _shares = maxOutData.shares;
        uint256 _minAmountOut = maxOutData.minAmountOut;
        ...
        assembly ("memory-safe") {
            ...
            mstore(0x20, _shares) // append the `shares` argument
            mstore(0x40, _recipient) // append the `recipient` argument
        ...
        address vaultAsset = maxOutData.vault.asset();
        if(maxOutData.assetOut != vaultAsset) {
           _swap(maxOutData.router, maxOutData.assetOut, maxOutData.swapData);
           amountOut = _thisBalance(maxOutData.assetOut);
        }

        assembly ("memory-safe") {
            if lt(amountOut, _minAmountOut) {
    ...
    function maxOutNative(
        ...
        assembly ("memory-safe") {
            ...
            if iszero(
                call(gas(), _recipient, amountOut, 0x00, 0x00, 0x00, 0x00)
            ) {
    ...
    function _swap(
        ...
        _approveMax(_asset, address(router));
        (bool s, ) = router.call(_callData);
    ...
    function _approveMax(address _vault, address _token) internal {
        (bool s, bytes memory data) = _token.staticcall(
