...
import {IYearnCompounder} from "src/interfaces/IYearnCompounder.sol";

import {FixedPointMathLib as Math} from "solady/utils/FixedPointMathLib.sol";

/// @title YearnAjnaWETHCompoundStrategy
/// @author Adapted from https://github.com/Grandthrax/yearn-steth-acc/blob/master/contracts/strategies.sol
/// @notice `YearnAjnaWETHCompoundStrategy` supplies an underlying token into a generic Yearn V3 Vault,
/// and deposits the shares in a compounder vault
contract YearnAjnaWETHCompoundStrategy is BaseStrategy {
    ...
    IYearnCompounder public constant yearnAjnaCompounder = IYearnCompounder(0xb974598227660bEfe79a23DFC473D859602254aC);
    ////////////////////////////////////////////////////////////////
    ///                         ERRORS                           ///
    ////////////////////////////////////////////////////////////////
    ...
    function initialize(
        ...
        address(_yVault).safeApprove(address(yearnAjnaCompounder), type(uint256).max);
    ...
    function _invest(uint256 amount, uint256 minOutputAfterInvestment) internal returns (uint256 depositedAmount) {
        ...
        yearnAjnaCompounder.deposit(shares, address(this));
    ...
    function _divest(uint256 shares) internal returns (uint256 withdrawn) {
        yearnAjnaCompounder.redeem(shares,address(this), address(this));
    ...
    function _shareValue(uint256 shares) internal view returns (uint256) {
        return yVault.previewRedeem(yearnAjnaCompounder.previewRedeem(shares));
    }
    ...
    function _sharesForAmount(uint256 amount) internal view returns (uint256 shares) {
        return yearnAjnaCompounder.convertToShares(yVault.convertToShares(amount));
    }
    ...
    function _shareBalance() internal view returns (uint256 _balance) {
        return yearnAjnaCompounder.balanceOf(address(this));
    }
