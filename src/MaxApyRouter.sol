...
contract MaxApyRouter {
    ...
    function depositNative(IMaxApyVaultV2 vault, address recipient, uint256 minSharesOut) external payable returns (uint256) {
    ...
    function redeemNative(IMaxApyVaultV2 vault, uint256 shares, address recipient, uint256 minAmountOut) external returns (uint256) {
        // Cache `wrappedToken` and `vault` due to assembly's immutable access restrictions
        address cachedWrappedToken = address(wrappedToken);
        address cachedVault = address(vault);
        ...
        assembly ("memory-safe") {
            ...
            if lt(amountWithdrawn, minAmountOut) {
                // Throw the `InfuccicientAssets` error
                mstore(0x00, 0x071c2f2c)
                revert(0x1c, 0x04)
            }
    ...
    function _approveMax(address _vault, address _token) internal {
        (bool s, bytes memory data) = _token.staticcall(abi.encodeWithSignature("allowance(address,address)", address(this), _vault));
        if(!s) revert();
