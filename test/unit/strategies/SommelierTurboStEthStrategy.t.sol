...
contract SommelierTurboStEthStrategyTest is BaseTest, StrategyEvents {
    ...
    function testSommelierTurboStEth__PrepareReturn() public {
        ...
        assertEq(realizedProfit, 20 ether);
        assertEq(unrealizedProfit, 40 ether);
        assertEq(loss, 0);
        ...
        assertEq(realizedProfit, 59.941982487207544339 ether); // 59.94 ETH
        ...
        assertEq(realizedProfit, 5.994198248720754432 ether); // 5.9 ETH
        assertEq(unrealizedProfit, 59.970984227645060889 ether); // 59.97 ETH
        assertEq(loss, 0);
        assertEq(debtPayment, 0);
        vm.revertTo(beforeReturnSnapshotId);
        ...
        vm.startPrank(address(strategy));
        IERC20(CELLAR_WETH_MAINNET).transfer(makeAddr("random"), strategy.sharesForAmount(10 ether));
        ...
        assertEq(loss, 9.995164037940843482 ether);
        assertEq(debtPayment, 0);
