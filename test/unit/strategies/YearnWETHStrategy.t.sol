...
contract YearnWETHStrategyTest is BaseTest, StrategyEvents {
    ...
    function testYearnWETH__Initialization() public {
        ...
        assertEq(IERC20(WETH).allowance(address(_strategy), YVAULT_WETH_MAINNET), type(uint256).max);
    ...
    function testYearnWETH__PrepareReturn() public {
        ...
        (uint256 realizedProfit,uint256 unrealizedProfit, uint256 loss, uint256 debtPayment) = strategy.prepareReturn(1 ether, 0, 5000);
    ...
    function testYearnWETH__LiquidateAllPositions() public {
        ...
        assertEq(IERC20(WETH).balanceOf(address(strategy)), strategyBalanceBefore + 9.999999999999999999 ether);
        /// 1 wei loss divesting
        assertEq(IERC20(YVAULT_WETH_MAINNET).balanceOf(address(strategy)), 0);
        ...
        /// 1 wei loss divesting
        assertEq(IERC20(WETH).balanceOf(address(strategy)), strategyBalanceBefore + 499.999999999999999999 ether);
        /// 1 wei loss divesting
        assertEq(IERC20(YVAULT_WETH_MAINNET).balanceOf(address(strategy)), 0);
    }
    ...
    ///                     TEST harvest()                       ///
    ////////////////////////////////////////////////////////////////
    function testYearnWETH__Harvest_Negatives() public {
        vault.addStrategy(address(strategy), 4000, type(uint72).max, 0, 0);
    ...
    function testYearnWETH__Harvest() public {
        ...
        vm.expectEmit();
        emit Harvested(4.523000000000000000 ether, 0, 0, 0);
        /// dont report any profit
        strategy.harvest(0, 0, 4523);
        /// vault balance doesnt increase at all                    // 4.52 ether
        assertEq(IERC20(WETH).balanceOf(address(vault)), 60 ether + 4.523000000000000000 ether );
        ...
        strategy.harvest(0, 0, 5000);
        assertEq(IERC20(WETH).balanceOf(address(vault)), 109.999999999999999999 ether);
        assertEq(IERC20(YVAULT_WETH_MAINNET).balanceOf(address(strategy)), 0);
        ...
        vm.expectEmit();
        emit Harvested(0, 1 wei, 2.991000000000000000 ether, 0);
        /// 10 ETH loss
        ...
        uint256 expectedShareDecrease = strategy.sharesForAmount(2.991000000000000000 ether);
        // here requesting 20% wont have any effect neither
        strategy.harvest(0, 0, 2000);

        data = vault.strategies(address(strategy));

        assertEq(vault.debtRatio(), 3001);
        assertEq(vault.totalDebt(), 27.009000000000000000 ether);
        assertEq(data.strategyDebtRatio, 3001);
        assertEq(data.strategyTotalDebt, 27.009000000000000000 ether);
        assertEq(data.strategyTotalLoss, 10 ether);
        assertEq(IERC20(WETH).balanceOf(address(vault)), vaultBalanceBefore + 2.991000000000000000 ether);
        assertLe(
    ...
    function testYearnWETH__PreviewWithraw() public {
     ...
     function testYearnWETH__PreviewWithrawRequest() public {
        ...
        strategy.harvest(0,0,0);
        vm.stopPrank();                                          
        uint256 requestedAmount = strategy.previewWithdrawRequest(23481322349392);
        vm.startPrank(address(vault));
        uint256 loss = strategy.withdraw(requestedAmount);
        assertEq(loss ,requestedAmount - 23481322349392);
    }
