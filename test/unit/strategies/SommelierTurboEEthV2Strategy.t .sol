...
contract SommelierTurboEEthV2StrategyTest is BaseTest, StrategyEvents {
    ...
    /*    function testSommelierTurboEEthV2__PreviewLiquidate__FUZZY(uint256 amount) public {
    ...
    /*     function testSommelierTurboEEthV2__PreviewLiquidateExact__FUZZY(uint256 amount) public {
        vm.assume(amount >= 0.0001 ether && amount <= 1000 ether);
        vault.addStrategy(address(strategy), 10_000, type(uint72).max, 0, 0);
        deal(WETH_MAINNET, users.alice, amount * 2);
        vault.deposit(amount * 2,users.alice);       
    ...
    /*     function testSommelierTurboEEthV2__maxLiquidateExact__FUZZY(uint256 amount) public {
        vm.assume(amount >= 0.0001 ether && amount <= 1000 ether);
        vault.addStrategy(address(strategy), 10_000, type(uint72).max, 0, 0);
        deal(WETH_MAINNET, users.alice, amount * 2);
        vault.deposit(amount * 2,users.alice);       
    ...
    /*     function testSommelierTurboEEthV2__MaxLiquidate__FUZZY(uint256 amount) public {
