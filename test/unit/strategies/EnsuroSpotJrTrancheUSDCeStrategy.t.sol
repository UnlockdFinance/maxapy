...
import { EnsuroSpotJrTrancheUSDCeStrategyWrapper } from "../../mock/EnsuroSpotJrTrancheUSDCeStrategyWrapper.sol";
import { _1_USDCE } from "test/helpers/Tokens.sol";
import { SafeTransferLib } from "solady/utils/SafeTransferLib.sol";

contract EnsuroUSDCeStrategyTest is BaseTest, ConvexdETHFrxETHStrategyEvents {
    using SafeTransferLib for address;

    address public TREASURY;
    IStrategyWrapper public strategy;
    EnsuroSpotJrTrancheUSDCeStrategyWrapper public implementation;
    ...
    function setUp() public {
        ...
        implementation = new EnsuroSpotJrTrancheUSDCeStrategyWrapper();
        ...
        TransparentUpgradeableProxy _proxy = new TransparentUpgradeableProxy(
            ...
            abi.encodeWithSignature(
                ...
                users.alice,
                ENSURO_POLICY_POOL_POLYGON,
                ENSURO_SPOT_JUNIOR_TRANCHE_ETOKEN_POLYGON,
                ENSURO_LP_MANUAL_WHITELIST_ADDRESS_POLYGON
            )
        );
    ...
    function testEnsuroUSDCe__Initialization() public {
        ...
        EnsuroSpotJrTrancheUSDCeStrategyWrapper _implementation = new EnsuroSpotJrTrancheUSDCeStrategyWrapper();
    ...
    function testEnsuroUSDCe__SetEmergencyExit() public {
        vm.stopPrank();
    ...
    function testEnsuroUSDCe__SetMinSingleTrade() public {
        vm.stopPrank();
        vm.startPrank(users.bob);
        vm.expectRevert(abi.encodeWithSignature("Unauthorized()"));
    ...
    function testEnsuroUSDCe__IsActive() public {
        ...
        strategy.divest(IERC20(ENSURO_SPOT_JUNIOR_TRANCHE_ETOKEN_POLYGON).balanceOf(address(strategy)));
    ...
    function testEnsuroUSDCe__SetStrategist() public {
        // Negatives
        vm.startPrank(users.bob);
        vm.expectRevert(abi.encodeWithSignature("Unauthorized()"));
    ...
    function testEnsuroUSDCe__InvestmentSlippage() public {
    ...
    function testEnsuroUSDCe__PrepareReturn() public {
    ...
    function testEnsuroUSDCe__Invest() public {
        // uint256 returned = strategy.invest(0, 0);
        // assertEq(returned, 0);
        // assertEq(IERC20(ENSURO_SPOT_JUNIOR_TRANCHE_ETOKEN_POLYGON).balanceOf(address(strategy)), 0);
        ...
        console2.log(
            "###   ~ file: EnsuroSpotJrTrancheUSDCeStrategy.t.sol:284 ~ testEnsuroUSDCe__Invest ~ expectedShares:",
            expectedShares
        );
        ...
        // assertApproxEq(
        //     expectedShares, IERC20(ENSURO_SPOT_JUNIOR_TRANCHE_ETOKEN_POLYGON).balanceOf(address(strategy)),
        // expectedShares / 100
        // );
    }

    function testEnsuroUSDCe__Divest() public {
        deal({ token: USDCE_POLYGON, to: address(strategy), give: 10 * _1_USDCE });
        uint256 expectedShares = strategy.sharesForAmount(10 * _1_USDCE);
        ...
        assertApproxEq(
            expectedShares,
            IERC20(ENSURO_SPOT_JUNIOR_TRANCHE_ETOKEN_POLYGON).balanceOf(address(strategy)),
            expectedShares / 100
        );
    ...
    function testEnsuroUSDCe__LiquidatePosition() public {
        deal({ token: USDCE_POLYGON, to: address(strategy), give: 10 * _1_USDCE });
    ...
    function testEnsuroUSDCe__LiquidateAllPositions() public {
        uint256 snapshotId = vm.snapshot();
        ...
        assertApproxEq(
            IERC20(ENSURO_SPOT_JUNIOR_TRANCHE_ETOKEN_POLYGON).balanceOf(address(strategy)), shares, shares / 100
        );
        ...
        assertApproxEq(
            IERC20(ENSURO_SPOT_JUNIOR_TRANCHE_ETOKEN_POLYGON).balanceOf(address(strategy)), shares, 1.5 ether
        );
    ...
    function testEnsuroUSDCe__Harvest() public {
        vm.expectRevert(abi.encodeWithSignature("Unauthorized()"));
        strategy.harvest(0, 0, address(0), block.timestamp);
    ...
    function testEnsuroUSDCe__PreviewLiquidate() public {
        vault.addStrategy(address(strategy), 4000, type(uint72).max, 0, 0);
        vault.deposit(100 * _1_USDCE, users.alice);
        vm.startPrank(users.keeper);
    ...
    function testEnsuroUSDCe__PreviewLiquidateExact() public {
        vault.addStrategy(address(strategy), 4000, type(uint72).max, 0, 0);
        vault.deposit(100 * _1_USDCE, users.alice);
        vm.startPrank(users.keeper);
        strategy.harvest(0, 0, address(0), block.timestamp);
    ...
    function testEnsuroUSDCe__maxLiquidateExact() public {
    ...
    function testEnsuroUSDCe__MaxLiquidate() public {
        vault.addStrategy(address(strategy), 9000, type(uint72).max, 0, 0);
        vault.deposit(100 * _1_USDCE, users.alice);
        vm.startPrank(users.keeper);
        strategy.harvest(0, 0, address(0), block.timestamp);
